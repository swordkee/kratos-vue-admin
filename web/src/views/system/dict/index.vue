    ...
    <el-card shadow="always">
      ...
      <el-form :model="state.queryParams" ref="queryForm" :inline="true" label-width="68px">
        ...
        <el-form-item label="字典类型" prop="dictType">
          <el-input v-model="state.queryParams.dictType" placeholder="请输入字典类型模糊查询" clearable style="width: 240px"
            @keyup.enter="handleQuery" />
        </el-form-item>
        <el-form-item label="状态" prop="status">
          <el-select v-model="state.queryParams.status" placeholder="字典状态" clearable style="width: 240px">
    ...
    <el-card class="box-card">
      <template #header>
        <div class="card-header">
          ...
          <div>
            ...
            <el-button type="danger" plain v-auth="'system:dictT:delete'" :disabled="state.multiple"
              @click="onTabelRowDel">
      ...
      <el-table v-loading="state.loading" border :data="state.tableData" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="状态" align="center" prop="status">
          <template #default="scope">
            <el-tag :type="scope.row.status === '1' ? 'danger' : 'success'" disable-transitions>{{ statusFormat(scope.row)
            }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column label="操作" align="center" width="350" class-name="medium-padding fixed-width">
          <template #default="scope">
