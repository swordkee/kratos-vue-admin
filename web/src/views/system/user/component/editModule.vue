<template>
  <div class="system-menu-container">
    <el-dialog v-model="state.isShowDialog" width="769px" center>
      ...
      <el-form ref="ruleFormRef" :model="state.ruleForm" :rules="state.ruleRules" label-width="110px">
        <el-row :gutter="35">
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
            <el-form-item label="用户性别" prop="sex">
              <el-select v-model="state.ruleForm.sex" placeholder="请选择">
                <el-option v-for="dict in state.sexOptions" :key="dict.dictValue" :label="dict.dictLabel"
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
            <el-form-item label="角色">
              <el-select v-model="state.roleIds" multiple placeholder="请选择">
                <el-option v-for="item in state.roleOptions" :key="parseInt(item.roleId)" :label="item.roleName"
                  :value="parseInt(item.roleId)" :disabled="item.status == 0"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
            <el-form-item label="归属部门" prop="deptId">
              <el-cascader v-model="state.ruleForm.deptId" :options="state.deptOptions" :props="{
                label: 'deptName',
                value: 'deptId',
                checkStrictly: true,
                emitPath: false,
              }" class="w100" clearable filterable placeholder="请选择归属部门" :show-all-levels="false"></el-cascader>
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
            <el-form-item label="岗位">
              <el-select v-model="state.postIds" multiple placeholder="请选择">
                <el-option v-for="item in state.postOptions" :key="parseInt(item.postId)" :label="item.postName"
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
            <el-form-item label="状态">
              <el-radio-group v-model="state.ruleForm.status">
                <el-radio v-for="dict in state.statusOptions" :key="parseInt(dict.dictValue)"
...
const openDialog = (row: any) => {
  if (row && row.userId && row.userId != undefined && row.userId != 0) {
    getUser(row.userId).then((response: any) => {
      ...
      state.postIds = response.postIds.split(",").map((item: string) => {
        return Number(item)
      });
      state.roleIds = response.roleIds.split(",").map((item: string) => {
        return Number(item)
      });
  ...
  proxy.getDicts("sys_user_sex").then((response: any) => {
    ...
    state.sexOptions = response.data?.list;
  });
  // 查询显示狀態数据字典
...
const getTreeselect = async () => {
  treeselect().then((response) => {
    state.deptOptions = response.data?.data;
  });
};
