<template>
  <div style="margin: 10px">
    <el-descriptions title="设备信息" :column="2" border>
      <el-descriptions-item label="设备编号" align="center" width="100">{{state.deviceOptions.ID}}</el-descriptions-item>
      <el-descriptions-item label="设备名" align="center" width="100">{{state.deviceOptions.Name}}</el-descriptions-item>
      <el-descriptions-item label="厂家" align="center" width="100">{{state.deviceOptions.Manufacturer}}</el-descriptions-item>
      <el-descriptions-item label="型号" align="center" width="100">{{state.deviceOptions.Model}}</el-descriptions-item>
      <el-descriptions-item label="设备地址" align="center" width="100">{{state.deviceOptions.Addr}}</el-descriptions-item>
     <el-descriptions-item label="注册时间" align="center">
        {{ dateStrFormat( state.deviceOptions.RegisterTime ) }}
    ...
    <el-table
        ...
        border
        :data="state.deviceOptions.Channels"
    >
  ...
  <el-dialog v-model="state.isShowZbDialog" width="769px" title="监控数据" center draggable @closed="closeDialog">
...
const handleQuery = () => {
  ...
  service({
    ...
    res.forEach((item)=>{
      if (item.ID === props.rowData.name){
        state.deviceOptions = item;
        // todo 设备状态 更改监控的设备状态
