    ...
    <el-card shadow="always">
      ...
      <el-form :model="state.queryParams" ref="queryForm" :inline="true" label-width="68px">
        <el-form-item label="表名称" prop="tableName">
          <el-input placeholder="请输入表名称模糊查询" clearable @keyup.enter="handleQuery" style="width: 240px"
            v-model="state.queryParams.tableName" />
        </el-form-item>
        <el-form-item label="表描述" prop="tableComment">
          <el-input placeholder="请输入表描述模糊查询" clearable @keyup.enter="handleQuery" style="width: 240px"
    ...
    <el-card class="box-card">
      <template #header>
        <div class="card-header">
          ...
          <div>
            <el-button type="primary" plain @click="onOpenAddModule" v-auth="'develop:code:add'">
      ...
      <!--数据表格-->
      <el-table v-loading="state.loading" :data="state.tableList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="创建时间" align="center" prop="createTime">
          <template #default="scope">
            <span>{{ dateStrFormat(scope.row?.createdAt) }}</span>
          </template>
        </el-table-column>
        <el-table-column label="更新时间" align="center" prop="updateTime">
          <template #default="scope">
            <span>{{ dateStrFormat(scope.row?.updatedAt) }}</span>
          </template>
        </el-table-column>
        <el-table-column label="操作" align="center">
          <template #default="scope">
            <el-popover placement="left">
              ...
              <div>
                <el-button text type="primary" v-auth="'develop:code:view'" @click="handlePreview(scope.row)">
                  <SvgIcon name="elementView" />
                  预览
                </el-button>
              </div>
              <div>
                <el-button text type="primary" v-auth="'develop:code:edit'" @click="onOpenEditModule(scope.row)">
                  <SvgIcon name="elementEdit" />
                  参数编辑
                </el-button>
              </div>
              <div>
                <el-button slot="reference" text type="primary" v-auth="'develop:code:code'"
              ...
              <div>
                <el-button slot="reference" text type="primary" @click="openConfigureDialog(scope.row)">
                  <SvgIcon name="elementView" />生成配置
                </el-button>
              </div>
              <div>
                <el-button text type="primary" v-auth="'develop:code:delete'" @click="onTabelRowDel(scope.row)">
                  <SvgIcon name="elementDelete" />
                  删除
                </el-button>
              </div>
    ...
    <el-dialog v-model="state.isShowConfigureDialog" width="769px" center>
      <el-form :model="state.configureForm" label-width="80px">
        <el-form-item label="上级菜单" prop="parentId">
          <el-cascader v-model="state.configureForm.parentId" :options="state.menuOptions" class="w100" :props="{
            label: 'menuName',
            value: 'menuId',
            checkStrictly: true,
            emitPath: false,
