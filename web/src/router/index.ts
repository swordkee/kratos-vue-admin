...
import { staticRoutes, newStaticRoutes } from '@/router/route';
...
export async function initBackEndControlRoutes() {
	...
	// 添加静态的路由
	// drs[0].children = drs[0].children?.concat(newStaticRoutes)
	// 添加404界面
...
function setMete(obj: any) {
	let meta = {
		auth: obj.auth || [],
		icon: obj.icon,
		isAffix: obj.isAffix === 1 ? true : false,
		isHide: obj.hideen === 1 ? true : false,
		isIframe: obj.isIframe === 1 ? true : false,
		isKeepAlive: obj.keepAlive === 1 ? true : false,
		isLink: obj.link === 1 ? true : false,
		title: obj.menuName,
	}
	obj.meta = meta
}
